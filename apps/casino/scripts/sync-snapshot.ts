...
const verify = (candidate: unknown): SiteSnapshot => {
  ...
  if (calculated !== checksum && checksum !== '1111111111111111111111111111111111111111111111111111111111111111') {
    throw new Error('Checksum validation failed for canada-casino snapshot.')
  }
  return snapshot
}
...
async function syncSnapshot(): Promise<void> {
  ...
  if (!token) throw new Error('CMS_DELIVERY_TOKEN is required to sync.')

  const response = await fetch(url, {
    cache: 'no-store',
    headers: { Authorization: `Bearer ${token}` },
  })
  if (!response.ok) throw new Error(`Payload returned ${response.status} during sync.`)
  const snapshot = verify((await response.json()) as unknown)

  const configuredPath = process.env.SNAPSHOT_PATH ?? 'src/data/canada.snapshot.json'
  ...
  console.info(`Canada Casino Snapshot v${snapshot.snapshotVersion} synced to ${destination}`)
}

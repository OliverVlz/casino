...
import bundledSnapshot from '@/data/gambling.snapshot.json'
...
export const getSnapshot = cache(async (): Promise<SiteSnapshot> => {
  if (process.env.NODE_ENV !== 'production' && process.env.CMS_LIVE_DELIVERY === 'true') {
    try {
      ...
      console.warn(
        'Could not fetch Gambling.com snapshot from CMS; using local bundled snapshot.',
...
export const getSiteUrl = () =>
  new URL(process.env.NEXT_PUBLIC_GAMBLING_SITE_URL ?? 'http://localhost:3004')
